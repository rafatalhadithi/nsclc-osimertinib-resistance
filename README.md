...
## Data Availability
Due to GitHub file size limits, raw `.fastq`, `.mtx`, and large `.h5ad` files are not included in this repository. 
...
1. Download `GSE131907_FINAL_ANNOTATED.h5ad` from: **[INSERT ZENODO SECRET LINK HERE]**
...
To align with the manuscript's formatting, several output files generated by this codebase have been renamed in the publication as Supplementary Data. 

If you run the analysis scripts, the code will output the files using their original **Codebase File Names**. Please use the table below to cross-reference the script outputs with the manuscript's supplementary tables:

| Manuscript File Name | Codebase File Name | Type | Description |
| :--- | :--- | :--- | :--- |
| `Supplementary Data 1.csv` | `Supplementary_Data_1_DGE_Cleaned.csv` | Output | Differential gene expression analysis results (log2 fold-changes and p-values). |
| `Supplementary Data 2.csv` | `osimertinib_resistance_pathways.csv` | Output | Pathway and Gene Ontology (GO) enrichment analysis results. |
| `Supplementary Data 3.csv` | `Reviewer_PAGA_Evolutionary_Connectivity.csv` | Output | PAGA evolutionary trajectory and connectivity matrix across distinct cellular compartments. |
| `Supplementary Data 4.csv` | `Reviewer_CV_Metrics.csv` | Output | Cross-validation performance metrics (Accuracy, F1, Precision, Recall) for the Random Forest classifier. |
| `Supplementary Data 5.csv` | `Reviewer_Feature_Importances.csv` | Output | Top biomarker genes and their extracted Random Forest importance weights. |
| `Supplementary Data 6.csv` | `Epigenetic_Driver_Correlations.csv` | Output | Spearman correlation metrics between epigenetic regulators (e.g., EZH2) and resistance status. |
| `Supplementary Data 7.csv` | `Pseudo_Bulk_Masking_Simulation.csv` | Output | Evaluation of the HOXC locus correlations comparing true spatial micro-environments vs. simulated bulk grinding. |
| `Supplementary Data 8.csv` | `Cross_Cohort_EZH2_Variance.csv` | Output | Cross-cohort validation of EZH2 and HOXC expression across 2D baseline, 2D treated, and 3D organoid DTP models. |
...
* **Output Files:** * `../data/marker_genes_complete.csv`
...
* **Output Files:** * `../data/Supplementary_Data_1_DGE_Cleaned.csv`
  * `../data/osimertinib_resistance_pathways.csv`
...
* **Output Files:** * `../data/Supplementary_Data_2_PPI_Edges.csv`
...
* **Output Files:** * `../data/GSE267960_Processed_Spatial.h5ad`
...
* **Input Files:** * `../data/Pfeifer_et_al_Supplemental_Data/MAGECK_files/PC9_Osimertinib_CRISPRn.txt`
  * `../data/validation_cohorts/GSE267960/GSM8282531_S20-12254-L9/S20-12254-L9/filtered_feature_bc_matrix.h5`
* **Output Files:** * `../data/CRISPR_Functional_Validation.csv`
...
* **Output Files:** * `../data/Reviewer_CV_Metrics.csv`
  * `../data/Reviewer_Feature_Importances.csv`
  * `../data/Reviewer_PAGA_Evolutionary_Connectivity.csv`
